...
# TODO(bchetioui): Change ``expected_transforms`` to be a ``Sequence``.
...
@_register_lowering(vector.InsertStridedSliceOp)
...
@_register_lowering(mgpu.AsyncStoreSmemToTmemOp, support_warp_semantics=True)
...
for _op, _binary_impl, _is_signed in [
    ...
    (arith.DivSIOp, lambda x, y: x.trunc_div(y), True),
    (arith.DivUIOp, lambda x, y: x.trunc_div(y), False),
...
def _memref_subview_op_lowering_rule(
  ...
  if swizzle != mgpu.SwizzlingMode.kNoSwizzle:
    ...
    for stride, offset, size in zip(
      ...
      if ir.ShapedType.is_dynamic_size(offset):
        dyn_offset = op.offsets[dyn_offset_index]
        dyn_offset_index += 1
        if not utils.is_known_divisible(dyn_offset, swizzle_elems):
          raise ValueError(
            f"Dynamic subview is not a multiple of {swizzle_elems=}."
        )
