...
def _rewrite_jaxpr_for_lowering(
  ...
  # a (1,) shaped Ref as an input. On TC, this Ref will be in SMEM and on SCS,
  # it will be in SMEM (TEC not supported yet).

  # On TC we will read the scalar values from SMEM and then eval the original
  # jaxpr with those values as an input. On SCS we will allocate SMEM for each
  # of the scalars, issue a sync copy, then do the same thing as TC.

  # We'll return a bool for each input to indicate whether the input was a scalar
  # that needed to be rewritten. This will tell the lowering to expand dims that
  # input to (1,).
  ...
  for is_scalar, v in zip(is_scalar_input, jaxpr.invars):
    if is_scalar:
      ...
      new_in_avals.append(
          state.AbstractRef(
              jax_core.ShapedArray((1,), v.aval.dtype), memory_space=mem_space
          )
      )
    else:
